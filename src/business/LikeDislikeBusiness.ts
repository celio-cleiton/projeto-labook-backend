    ...
    public async editLikeDislikeId(input: EditPostLikesInputDTO): Promise<void> {
        ...
        if (!likesDislikesDB) {
            ...
            if (updatedLike === Boolean(like)) {
                ...
                const updatedLikeValue = Number(!like);
                const updatedLikesDislikes = new LikeDislike(userId, postId, updatedLikeValue);
                ...
                deltaLikes = updatedLikeValue ? 1 : -1;
                deltaDislikes = updatedLikeValue ? -1 : 1;
            }
        }
